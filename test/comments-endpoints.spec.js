...
describe('Comments Endpoints', function() {
  ...
  describe(`POST /api/comments`, () => {
    ...
    it(`creates an comment, responding with 201 and the new comment`, function() {
      this.retries(3)
      const testAffirmation = testAffirmations[0]
      ...
      return supertest(app)
        ...
        .expect(res => {
          ...
          expect(res.body.affirmation_id).to.eql(newComment.affirmation_id)
        ...
        .expect(res =>
          db
            .from('capstone1_comments')
            .select('*')
            .where({ id: res.body.id })
            .first()
            .then(row => {
              expect(row.content).to.eql(newComment.content)
              expect(row.affirmation_id).to.eql(newComment.affirmation_id)
    ...
    const requiredFields = ['content', 'affirmation_id']
